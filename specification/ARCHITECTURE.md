...
- `EmotionFrame{emotion, intensity, gaze, accent_color?, speaking, ttl_ms}` (from v2) — emitted per turn/state change; `emotion` is a small fixed enum (see §Emotion face / EMOTION_FACE.md), extended with **MCP-driven emotions** (searching / recalling / consulting / eureka / puzzled) in v3.8; `intensity` 0–1. Decided server-side, rendered on the device.
