...
The device is one of a **family of M5Stack boards**, not a single SKU: v1 targets **AtomS3R + Echo Base** (ES8311 audio, 128×128 LCD); later targets — **AtomS3R + Echo Pyramid base** (adds a mic array and an addressable LED halo) and **Core S3** — add capabilities the firmware detects and uses when present, degrading gracefully when absent.
...
- Planned but not early: the **emotion face** (from v2), **additional hardware** beyond Echo Base (Echo Pyramid base, Core S3), and **web search** (v3). Each lands in its version, not before.
