...
- **Icon sprites (v3.8):** procedural layered sprites + idle loop + crossfade + lip-sync.
...
- **v4.1 — emotion halo (Echo Pyramid base).** The WS2812 halo becomes a second
  ...
  pulse) — see §9. No new contract; reuses the v2.5 `emotion` message.
- **v3.8 — sprite face (Icon).** Swap `EmojiRenderer` → `IconRenderer`: the layer
...
- firmware: `IFaceRenderer` + `EmojiRenderer` (v2.5), `IconRenderer` (v3.8),
